...
function renderInitialDetail(model){const root=document.getElementById('detail'); const h=document.createElement('h3'); text(h,'All-library valence / arousal / BPM graph'); const p=document.createElement('p'); text(p,`Showing ${visibleGraph.nodes.length} positioned tracks and ${visibleGraph.unpositioned.length} unpositioned tracks. X=native valence, Y=native arousal, Z=raw BPM (fixed 20 BPM per depth unit); selected mood ${model.selectedMood||'none'} only changes the score strip; no sample-relative clipping.`); const graph=document.createElement('section'); graph.className='field'; graph.append(text(document.createElement('h4'),'Graph key / status'),graphStatusParagraph(visibleGraph)); graph.append(text(document.createElement('p'),graphColorLegend(model)),text(document.createElement('p'),graphRelatednessLegend())); const help=document.createElement('p'); help.className='muted'; text(help,'3d-force-graph is bundled locally. Fixed coordinates are scaled for readable display only. Drag to orbit, wheel to zoom, hover/click nodes and links for evidence. Sparse edges are axis-independent relatedness from existing summaries, not screen distance.'); const list=document.createElement('ul'); for(const u of model.unpositioned){const li=document.createElement('li'); text(li,`${u.display_label||u.track_id}: ${(u.reasons||[]).join('; ')}`); list.append(li);} root.replaceChildren(h,p,graph,help,list);}
...
function graphStatusText(model=graphModel){const nodes=(model.nodes||model.positioned||[]).length; const links=(model.links||model.edges||[]).length; const unpositioned=(model.unpositioned||[]).length; return `${nodes} positioned tracks · ${links} sparse relatedness edges · ${unpositioned} unpositioned`; }
function graphStatusParagraph(model=graphModel){const p=document.createElement('p'); text(p,graphStatusText(model)); return p;}
...
function renderTrackMetadata(metadata){const section=document.createElement('section'); section.className='field'; section.append(text(document.createElement('h4'),'Embedded file metadata')); const common=metadata&&Array.isArray(metadata.common)?metadata.common:[]; const tags=metadata&&Array.isArray(metadata.tags)?metadata.tags:[]; const warnings=metadata&&Array.isArray(metadata.warnings)?metadata.warnings:[]; if(!common.length&&!tags.length) section.append(text(document.createElement('p'),'No embedded textual metadata found; using filename display label.')); for(const [k,v] of common){section.append(text(document.createElement('p'),`${k}: ${Array.isArray(v)?v.join('; '):v}`));} if(tags.length){const details=document.createElement('details'); details.append(text(document.createElement('summary'),`All supported textual tags (${tags.length})`)); for(const [k,vals] of tags) details.append(text(document.createElement('p'),`${k}: ${(vals||[]).join('; ')}`)); section.append(details);} for(const warning of warnings) section.append(text(document.createElement('p'),warning)); return section;}
function renderDetail(d){const root=document.getElementById('detail'); const selected=graphModel.nodes.find(n=>n.id===d.handle); const score=document.createElement('p'); const selectedScore=selected?.moodScore; text(score,selectedScore?`Selected ${selectedScore.label} raw sigmoid mean score: ${displayNumber(selectedScore.raw)} / 1 (not a calibrated probability).`:`Selected ${graphModel.selectedMood||'mood'} score: unavailable for this track.`); const heading=document.createElement('h3'); text(heading,'Current track'); const identity=document.createElement('p'); text(identity,`Display label: ${d.display_label||d.handle} · Stable identifier: ${d.track_id||d.handle}`); const status=document.createElement('p'); text(status,`Status: ${d.latest_run_status||'missing'} · Locations: ${d.available_locations}`); const graph=document.createElement('section'); graph.className='field'; graph.append(text(document.createElement('h4'),'Graph key / status'),graphStatusParagraph(visibleGraph),text(document.createElement('p'),graphColorLegend(graphModel)),text(document.createElement('p'),graphRelatednessLegend())); const info=document.getElementById('graph-info'); if(info) text(info,'Graph key and status are shown in Current / graph notes.'); const reasons=document.createElement('p'); reasons.className='muted'; text(reasons,(d.reasons||[]).join('; ')); const fields=document.createElement('div'); fields.append(renderTrackMetadata(d.metadata)); for(const [name,f] of Object.entries(d.fields||{})) fields.append(renderDetailField(name,f)); root.replaceChildren(heading,identity,status,score,graph,reasons,fields);}
...
if(typeof module!=='undefined'){module.exports={detailGauge,displayNumber,graphAxisSpec,syncGraphAxes,buildMoodStrip,nodeLabel,buildMoodGraphModel,moodNodeColor,graphColorLegend,graphQueryFromControls,applyMoodGraphFilters,passesGraphFilters,buildGraphModel,applyGraphFilters,orbitCamera,panCamera,zoomCamera,canvasPoint,fieldDisplay,graphDimensions,updateGraphSize,graphCameraFrame,selectionClientId,syncSelectionEpoch,renderInitialDetail,renderDetail,graphStatusText,graphStatusParagraph,renderTrackMetadata,setGraphModelForTesting,setVisibleGraphForTesting,graphRelatednessLegend,relatednessLinkAlpha};}
