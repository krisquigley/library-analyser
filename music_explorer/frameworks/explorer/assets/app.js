...
function renderDetail(d){const root=document.getElementById('detail'); const heading=document.createElement('h3'); text(heading,'Current Track'); const reasons=document.createElement('p'); reasons.className='muted'; text(reasons,(d.reasons||[]).join('; ')); const fields=document.createElement('div'); fields.append(renderTrackMetadata(d.metadata)); for(const [name,f] of Object.entries(d.fields||{})) fields.append(renderDetailField(name,f)); root.replaceChildren(heading,reasons,fields);}
