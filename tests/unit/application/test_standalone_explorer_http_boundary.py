...
class StandaloneExplorerHttpBoundaryTests(unittest.TestCase):
    def test_server_does_not_import_private_projection_artifact_builder(self):
        tree = ast.parse(Path('music_explorer/frameworks/explorer/server.py').read_text())
        private_imports = []
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom) and node.module == 'music_explorer.application.use_cases.projection_artifacts':
