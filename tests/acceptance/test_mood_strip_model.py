...
const node = (id, raw, mood='calm', bpm=100) => ({track_id:id,display_label:id,x:{raw:0.1,normalized:0.1},y:{raw:0.2,normalized:0.2},z:{label:mood,raw,normalized:raw*2-1,scale:'sigmoid-score-[0,1]->[-1,1]'},bpm});
const payload = {selected_mood:'calm',positioned:[node('z',0.6),node('b',0),node('a',0.6),node('top',1)],unpositioned:[{track_id:'missing',reasons:['missing mood']}]};
...
assert(app.nodeLabel(model.nodes[1]).includes('calm 0 ('));
